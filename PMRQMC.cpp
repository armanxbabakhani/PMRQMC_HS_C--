...
int FindCycles(int r){  // find all cycles of length between lmin and lmax, each containing all operators of the array Sq_subseq of length r.
	...
	vector<int> found_cycle_list;
	found_cycle_list.resize(Ncycles);
	...
	for(i=0;i<Ncycles;i++){
		...
		not_contained = 0; for(j=0;j<r;j++) if(!cycles[i][Sq_subseq[j]]){ not_contained = 1; break;}
		...
		if(q >= u+rmin){
			...
			if(NoRepetitionCheck(Sq_subseq,r)){
				...
				if(found_cycles > 0){ // cycles[m] is one of the found cycles, containing all the operators of Sq_subseq
					...
					oldq = q; P = cycles[m]; for(i=0;i<r;i++) P[Sq_subseq[i]] = 0;
					p = vector_sum(P); // here, p is length of the complement sequence S'
					...
					for(i=0;i<p;i++){ while(!P[j]) j++; Sq_subseq[i] = Sq[min_index+i] = j++;}
...
int main(int argc, char* argv[]){
	if(argc != 2){ cout << "Usage: ./PMRQMC.bin hamiltonian.txt" << endl; exit(1);}
	cout << "Preparing PMR for the Hamiltonian and computing the list of fundamental cycles...";
	string fileName(argv[1]); int i,k,o=0;
	...
	cout << "Main: Nop is " << Nop << endl;
	Ncycles = cyclesTr.size() == 0 ? 0 : cyclesTr[0].size();
	for(i=0;i<Ncycles;i++){
		cycles.push_back(vector<int>());
		for(k=0;k<Nop;k++) cycles[i].push_back(cyclesTr[k][i]);
	}
	cout << "done" << endl;
	...
	divdiff_init(); divdiff dd(q+4,500); d=&dd; init();
